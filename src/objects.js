...
SpriteMorph.prototype.blockColor = {
	...
	key: new Color(255, 41, 105, 0),
	token: new Color(255,255,255,0)
};
...
SpriteMorph.prototype.initBlocks = function () {
	SpriteMorph.prototype.blocks = {
		...
		receiveOnClone: {
			type: 'hat',
			category: 'control',
			spec: 'when I recieve %readonly',
			defaults: [['__clone__init__']]
		},
		createClone: {
			type: 'command',
			category: 'control',
		...
		reify: {
			type: 'predicate',
			category: 'operator',
			spec: 'ringify %obj with inputs %mult%s'
        },
		reifyScript: {
			type: 'ring',
			category: 'other',
		...
		reportScriptAsContext: {
			type: 'predicate',
			category: 'key',
			spec: 'function %staticin $( %ringparms $){ %c $}'
        },
		reportVariadicSum: {
			type: 'reporter',
			category: 'token',
...
SpriteMorph.prototype.blockAlternatives = {
	...
	// lists
	reportEmptyArray: ['reportNewArray'],
	reportNewArray: ['reportEmptyArray'],
...
SpriteMorph.prototype.blockTemplates = function (
	...
	if (category === 'motion') {
		...
		blocks.push('#');
		blocks.push(block('reportScriptAsContext'));
		blocks.push('-');
		...
		blocks.push(block('reportNew'));

		let userAgent = window.navigator.userAgent.toLowerCase(),
			macos = /(macintosh|macintel|macppc|mac68k|macos)/i,
			ios = /(iphone|ipad|ipod)/i;
		let isApple = macos.test(userAgent) || ios.test(userAgent)

		if ((!isApple) || Process.prototype.enableJS)
		...
		blocks.push(block('getProp'));
		blocks.push(block('getIndex'));
		blocks.push('-');

		blocks.push(block('doSetVar'));
		blocks.push(block('doChangeVar'));
		blocks.push(block('setProp'));
		blocks.push(block('setIndex'));
		...
		blocks.push(block('reportNewArray'));
