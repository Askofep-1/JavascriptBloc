...
Process.prototype.doTryCatch = function (action, errName, catchAction)
{
	this.tryCatch(this.reify(action,new List([]),false), this.reify(catchAction,new List([]),false), errName);
	this.resetErrorHandling();
};
...
Process.prototype.reportNew = function (cls,params)
{
	return new cls(...
		params.itemsArray().concat((cls.context instanceof Context) ? [this] : []))
}
...
Process.prototype.reportScriptAsContext = function (Name ,paramNames, top)
{
	var funct
	var context = this.reify(top, new List(["arguments", "this", ...paramNames.asArray()]));
	if (top == null) { /*funct.context = blank;*/ return new Function(paramNames.asArray().concat([""])); }
	funct = function (...args)
	{
		return funct.proc.evaluate(context, new List([arguments, this].concat(args.slice(0, -1))));
	};
	funct.context = context
	Object.defineProperty(funct, 'name', {
		value: Name
	})
	funct.toString = function ()
	{
		return "function "+this.name+"(" + this.context.inputs.slice(2).toString() + "){\n" + this.context.expression.toString() + "\n}";
	};
	funct.proc = this
	var ring = new RingMorph();
	ring.setSpec('%rc %ringparms');
	/*var blank = this.reify(ring, new List(["arguments", "this", ...paramNames.asArray()]));
	blank.expression.hide();*/
...
Process.prototype.reportJSFunction = function (parmNames, body)
{
	let userAgent = window.navigator.userAgent.toLowerCase(),
		macos = /(macintosh|macintel|macppc|mac68k|macos)/i,
		ios = /(iphone|ipad|ipod)/i ;

	if ((macos.test(userAgent) || ios.test(userAgent)) && !this.enableJS) {
...
Process.prototype.reportNewArray = function (items)
{
	let out = new Array();
	let i = 0
	for (const item of items.itemsArray())
	{
		out[i] = this.evaluate(this.reify(item, new List([])));
		i++;
	}
	return out;
}

Process.prototype.reportEmptyArray = function ()
{
	return new Array();
}
...
)
{
	...
	if (runnable.expression instanceof CommandBlockMorph)
	{
		...
		return context(...
			args.itemsArray().concat((context.context instanceof Context)?[this]:[])//apply(
//            this.blockReceiver(),
//            args.itemsArray().concat([this])
		);
	}
...
Process.prototype.reportGetVar = function () {
	...
	{ return eval(this.context.expression.blockSpec) }
};
...
Process.prototype.doDeleteFromList = function (index, list) {
	...
	if (list instanceof Array)
	{
		if (this.inputOption(index) === 'all') list.splice(0, list.length);
		if (this.inputOption(index) === 'last') list.splice(-1, 1);
...
Process.prototype.doInsertInList = function (element, index, list) {
	...
	if (list instanceof Array)
	{
		if (this.inputOption(index) === 'any') list.splice(math.round(math.random() * list.length), 0);
		if (this.inputOption(index) === 'last') list.splice(-1, 0,element);
		else list.splice(index, 0,element);
		return;
	}
	if (list.type) {
		this.assertType(element, list.type);
...
Process.prototype.doReplaceInList = function (index, list, element) {
	...
	if (list instanceof Array)
	{
		if (this.inputOption(index) === 'any') idx = this.reportBasicRandom(0, list.length);
		if (this.inputOption(index) === 'last') idx = -1;
		list[idx] = element;
		return;
	}
	if (list.type) {
	...
	if (this.inputOption(index) === 'last' || index <= 0) {
		idx = list.length();
	}
...
Process.prototype.doForEach = function (upvar, list, script) {
	...
	if (this.context.accumulator === null) {
		//this.assertType(list, 'list');
		this.context.accumulator = {
			source : (list instanceof List)?list:new List(list),
...
Process.prototype.doJSFor = function (statement, condition, statement2, body)
{
    for (this.evaluate(this.reify(statement, new List()), new List(), true); this.evaluate(condition, new List()); this.evaluate(this.reify(statement2, new List()), new List(), true))
	{
		this.evaluate(this.reify(body,new List()),new List(), true)
    }
}
...
Process.prototype.reportIsA = function (thing, type)
{
	return (thing instanceof type);
};

Process.prototype.getProp = function (obj, name)
{
	return obj[name]
}

Process.prototype.setProp = function (obj, name, val)
{
	obj[name] = val
}

Process.prototype.getIndex = function (obj, name)
{
	return obj[name]
}

Process.prototype.setIndex = function (obj, name, val)
{
	obj[name] = val
}
...
Process.prototype.reportNull = function ()
{
	return null;
};

Process.prototype.reportThis = function ()
{
	return this;
};
...
{
	this.uber.call(this, [[`
	var args = Array.splice.call(arguments)
	if (arguments.callee.isContinuation) {
		return this.runContinuation(context, args);
	}

	var outer = new Context(null, null, arguments.callee.outerContext),
		caller = arguments.caller.caller,
		exit,
		runnable,
		expr,
		parms = args,
	...
	runnable.isCustomCommand = isCommand; // for short-circuiting HTTP requests
	//this.context.parentContext = runnable; this will throw an error

	if (arguments.callee.expression instanceof ReporterBlockMorph) {
		// auto-"warp" nested reporters
		//this.readyToYield = (this.currentTime - this.lastYield > this.timeout);
	}
	...
	if (parms.length > 0) {
		...
		// assign formal parameters
		for (i = 0; i < arguments.callee.inputs.length; i += 1) {
			value = 0;
			if (!isNil(parms[i])) {
				value = parms[i];
			}
			outer.variables.addVar(arguments.callee.inputs[i], value);
		}

		// assign implicit parameters if there are no formal ones
		if (arguments.callee.inputs.length === 0) {
			// in case there is only one input
			// assign it to all empty slots...
			if (parms.length === 1) {
				// ... unless it's an empty reporter ring,
				// in which special case it gets treated as the ID-function;
				// experimental feature jens is not at all comfortable with
				if (!arguments.callee.emptySlots) {
					expr = arguments.callee.expression;
					if (expr instanceof Array &&
							expr.length === 1 &&
							expr[0].selector &&
							expr[0].selector === 'reifyReporter' &&
			...
			} else if (arguments.callee.emptySlots !== 1) {
				throw new Error(
					localize('expecting') + ' ' + context.emptySlots + ' '
						+ localize('input(s), but getting') + ' '
						+ parms.length
	...
	//this.isFromFunct = false;
}

Context.prototype = new Function();

Context.prototype.uber = Function;
...
function Variable(value, isTransient, isHidden,isConstant) {
	this.value = value;
...
{
	return new Variable(this.value, this.isTransient, this.isHidden, this.isConstant);
};
...
VariableFrame.prototype.setVar = function (name, value, sender) {
	...
	if (frame/* && !frame[name].isConstant */)
	{
